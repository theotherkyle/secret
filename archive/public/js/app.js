...
const options = {
  ...
  streamContainers: function streamContainers(pubSub, type, data) {
    return {
      publisher: {
        camera: '#cameraGuestPublisherContainer',
...
const app = function() {
   ...
   */
  const updatePublishersState = function(updates) {
   ...
   * Update the state and UI
   */
  const updateSubscribersState = function(updates) {
    Object.assign(state, updates);
    Object.keys(updates).forEach(update => updateSubscribersUI(update));
  };
...
const startGuestVideo= function() {
    otCore.startCall()
      .then(function({ publishers, subscribers, meta }) {
        ...
        updateSubscribersState({ subscribers, active: true });
        updatePublishersState({ publishers, meta, active: true });
